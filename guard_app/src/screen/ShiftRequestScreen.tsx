...
export default function ShiftRequestScreen() {
    ...
    const fetchData = () => {
        ...
        //DUMMY DATA
        const row1: ShiftRequest = { id: 'id', type: 'SWAP', status: 'PENDING', created_at: new Date().toDateString(), reason: 'The reason is I really want a day off', };
        const row2: ShiftRequest = { id: 'id2', type: 'LEAVE', status: 'APPROVED', created_at: new Date().toDateString(), reason: 'The reason is I am testing stuff', };
        const row3: ShiftRequest = { id: 'id3', type: 'SWAP', status: 'REJECTED', created_at: new Date().toDateString(), };
        const row4: ShiftRequest = { id: 'id4', type: 'LEAVE', status: 'PENDING', created_at: new Date().toDateString(), reason: 'The reason is I really want a day off', };
        const row5: ShiftRequest = { id: 'id5', type: 'SWAP', status: 'APPROVED', created_at: new Date().toDateString(), reason: 'The reason is I am testing stuff',};
        const row6: ShiftRequest = { id: 'id6', type: 'LEAVE', status: 'REJECTED', created_at: new Date().toDateString()};
    ...
    useFocusEffect(useCallback(() => setRows(fetchData()), []));
                    ...
                    <View style={styles.card}>
                        <View style={styles.cardHeader}>
                            <View style={styles.cardTitleSection}>
                                ...
                                <View style={[styles.cardStatusBadge, { backgroundColor: getStatusColor(item.status) }]}>
                                    <Text style={styles.cardStatusText}>
                                        {item.status === 'PENDING' ? t("shifts.pending") : item.status === "APPROVED" ? t("shifts.approved") : t("shifts.rejected")}
                                    </Text>
                                </View>
                            </View>
                        </View>
                        ...
                        <View style={styles.cardRow}>
                            ...
                            <Text style={styles.cardValue}>{item.type === 'LEAVE' ? t('shifts.leave') : t("shifts.swap")}</Text>
                        </View>
