...
const DEFAULT_ROLE_PERMISSIONS = {
  ...
  [ROLES.GUARD]: ["shift:read", "shift:accept", "shift:checkin", "shift:apply"],
  [ROLES.CLIENT]: ["shift:read", "payment:write"],
};
...
async function getEffectivePermissions(roleName, visited = new Set()) {
  ...
  if (roleDoc?.inheritsFrom) {
    const parentPerms = await getEffectivePermissions(
      roleDoc.inheritsFrom,
      visited,
    );
