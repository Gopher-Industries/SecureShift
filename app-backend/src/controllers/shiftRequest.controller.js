...
export const createShiftRequest = async (req, res) => {
    try {
        ...
        // Notify employer (creator of the shift)
        const shiftCreator = await User.findById(originalShift.createdBy);
        if (shiftCreator && shiftCreator.email) {
            await sendEmail({
                to: shiftCreator.email,
                subject: `Shift ${type.toLowerCase()} request from ${req.user.name}`,
                html: `
          <h3>New Shift Request</h3>
          <p><strong>Guard:</strong> ${req.user.name}</p>
          <p><strong>Type:</strong> ${type}</p>
          <p><strong>Shift:</strong> ${originalShift.title} on ${originalShift.date.toDateString()}</p>
          <p><strong>Reason:</strong> ${reason}</p>
          <p><a href="${process.env.FRONTEND_URL}/dashboard/requests/${shiftRequest._id}">View Request</a></p>
        `,
            });
        }
...
export const updateShiftRequest = async (req, res) => {
    try {
        ...
        const isTargetGuard = request.type === 'SWAP' &&
            request.targetGuardId?._id.toString() === req.user._id.toString();
        const isRequester = request.requestingGuardId._id.toString() === req.user._id.toString();
        ...
        if (targetResponse && request.type === 'SWAP' && isTargetGuard && request.status === 'PENDING') {
            ...
            // Notify requester
            await sendEmail({
                to: request.requestingGuardId.email,
                subject: `Swap request ${targetResponse === 'ACCEPTED' ? 'accepted' : 'declined'}`,
                html: `<p>Your swap request for shift ${request.originalShiftId.title} has been ${targetResponse === 'ACCEPTED' ? 'accepted' : 'declined'} by ${req.user.name}.</p>`,
            });
        ...
        if (status && (isAdmin || isEmployer) && request.status === 'PENDING') {
            ...
            if (status === 'APPROVED') {
                ...
                if (request.type === 'SWAP') {
                    ...
                    // Notify both guards
                    await sendEmail({
                        to: [request.requestingGuardId.email, targetGuard.email],
                        subject: 'Shift swap approved',
                        html: `<p>Your shift swap request has been approved by ${req.user.name}.</p>`,
                    });
                    ...
                    // Notify employer to find replacement
                    const employer = await User.findById(originalShift.createdBy);
                    if (employer) {
                        await sendEmail({
                            to: employer.email,
                            subject: `Leave approved - shift ${originalShift.title} needs replacement`,
                            html: `<p>${request.requestingGuardId.name} has been approved for leave from ${request.leaveStartDate} to ${request.leaveEndDate}. Shift ${originalShift.title} on ${originalShift.date} is now open.</p>`,
                        });
                    }
                }
                ...
                await sendEmail({
                    to: request.requestingGuardId.email,
                    subject: `Shift ${request.type.toLowerCase()} request rejected`,
                    html: `<p>Your ${request.type.toLowerCase()} request for shift ${request.originalShiftId.title} has been rejected.</p><p>Reason: ${request.rejectionReason}</p>`,
                });
            }
