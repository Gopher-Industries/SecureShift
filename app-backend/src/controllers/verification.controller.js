...
export const startVerification = async (req, res) => {
  try {
    const { guardId, jurisdiction, licenceNumber, firstName, lastName, dob } =
      req.body;
    if (!guardId || !jurisdiction || !licenceNumber) {
      return res
        .status(400)
        .json({
          message: "guardId, jurisdiction and licenceNumber are required",
        });
    }
    ...
    if (jurisdiction.toUpperCase() === "NSW") {
      ...
      if (!result.ok) {
        ...
        return res
          .status(200)
          .json({ message: "NSW verification attempted", verification: gv });
      }
      ...
      return res
        .status(200)
        .json({ message: "NSW verification result saved", verification: gv });
    } else {
      // Create manual verification record
      const manual = await createManualVerification({
        guardId,
        jurisdiction,
        notes: "Created via API fallback",
      });
      const gv = await GuardVerification.create({
        guardId,
        jurisdiction,
...
export const getStatus = async (req, res) => {
  try {
    ...
    if (
      String(requester.id) !== String(guardId) &&
      requester.role !== "admin"
    ) {
      return res.status(403).json({ message: "Forbidden" });
    }

    const latest = await GuardVerification.findOne({ guardId })
      .sort({ createdAt: -1 })
      .lean();
    if (!latest)
      return res
        .status(404)
        .json({ message: "No verification found for this guard" });
...
export const recheckVerification = async (req, res) => {
  try {
    ...
    const latest = await GuardVerification.findOne({ guardId }).sort({
      createdAt: -1,
    });
    if (!latest)
      return res
        .status(404)
        .json({ message: "No verification snapshot found" });
    ...
    if (latest.source === "nsw_api") {
      ...
      const manualIdNote = (latest.notes || "").match(
        /manualId:([0-9a-fA-F]{24})/,
      );
