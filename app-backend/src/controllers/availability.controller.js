...
export const createOrUpdateAvailability = async (req, res) => {
  try {
    ...
    if (bodyUserId) {
      ...
      if (requester.role === 'admin') {
        ...
        return res.status(403).json({
          message:
            'Forbidden: you can only update your own availability.',
        });
      }
    }
    ...
    if (
      ...
      return res.status(400).json({
        message:
          'Please provide at least one time slot.',
      });
    }
    ...
    if (err.code === 11000) {
      return res.status(409).json({
        message:
          'Availability already exists for this user.',
      });
    }
...
export const getAvailability = async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(403).json({
        message:
          'Forbidden: cannot access other user availability.',
      });
    }
    ...
    if (!availability) {
      return res.status(404).json({
        message:
          'Availability not found for this user.',
      });
    }
...
export const updateAvailabilityStatus = async (
  ...
  try {
    ...
    console.error(
      'Availability STATUS PATCH error:',
      err
    );
