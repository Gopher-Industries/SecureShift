...
function deriveUiStatus(record) {
  ...
  if (record.status === 'incomplete') {
    if (record.clockIn && record.scheduledStart) {
      const lateBy = new Date(record.clockIn).getTime() - new Date(record.scheduledStart).getTime();
...
function buildRows(shifts, attendanceLists) {
  return shifts.map((shift, i) => {
    ...
    const record = records.find((r) => r.guard?._id === guardId) || records[0] || null;
    ...
    const totalPayment =
      record?.clockOut && hoursWorked > 0 ? `$${(hoursWorked * payRate).toFixed(2)}` : '--';
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows
      .filter((r) => {
        ...
        const matchesSite = selectedSite === 'All Sites' || r.location === selectedSite;
        const matchesSearch =
          !q || r.guard.toLowerCase().includes(q) || r.location.toLowerCase().includes(q);
  ...
  return (
    <div className="timesheet-page">
      <div className="timesheet-container">
        ...
        {error && <div style={{ color: 'red', padding: '0.5rem 0' }}>{error}</div>}
