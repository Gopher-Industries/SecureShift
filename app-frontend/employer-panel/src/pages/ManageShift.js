...
const ManageShift = () => {
  ...
  const handleAddEquipment = () => {
    ...
    if (!trimmed) return;
    setEquipmentList((prev) => [...prev, { id: Date.now(), name: trimmed, condition: null }]);
    setNewEquipmentName('');
  };
              ...
              <div style={{ marginTop: '8px' }}>
                ...
                {selectedShift.status === 'In Progress' && (
                  <div style={equipmentAddRowStyle}>
                    ...
                    />
                    <button style={primaryButton} onClick={handleAddEquipment}>
                      Add
                    </button>
                  </div>
                )}
                  ...
                  >
                        ...
                        <div style={conditionButtonGroupStyle}>
                          {['Good', 'Damaged', 'Lost'].map((cond) => (
                            <button
                              ...
                              style={
                                item.condition === cond
                                  ? activeConditionStyle(cond)
                                  : inactiveConditionStyle
                              }
                    ...
                    {selectedShift.status === 'Completed' && (
                      <div style={equipmentSummaryStyle}>
                        <span>
                          ✅ Good: {equipmentList.filter((e) => e.condition === 'Good').length}
                        </span>
                        <span>
                          ⚠️ Damaged:{' '}
                          {equipmentList.filter((e) => e.condition === 'Damaged').length}
                        </span>
                        <span>
                          ❌ Lost: {equipmentList.filter((e) => e.condition === 'Lost').length}
                        </span>
...
const removeEquipmentButtonStyle = {
  background: 'none',
  border: 'none',
  fontSize: '20px',
  color: '#9ca3af',
