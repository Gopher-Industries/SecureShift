...
const ManageShift = () => {
    ...
    const handleApproveGuard = async (guardId) => {
        ...
        try {
            ...
            const updatedShift = normalizeShift(data.shift || {
                ...selectedShift,
                status: 'inprogress',
            ...
            const msg = err?.response?.data?.message || 'Failed to approve guard';
            setFeedback(msg);
    ...
    const handleCompleteShift = async () => {
        ...
        try {
            ...
            const msg = err?.response?.data?.message || 'Failed to complete shift';
            setFeedback(msg);
        } finally {
            setSaving(false);
        }
        ...
        <div style={applicantsPanelStyle}>
            {assignedGuardId && !isCompleted && (
                <div style={assignedBannerStyle}>
                    ✅ A guard has been approved. Shift is In Progress.
                </div>
            )}
            {isCompleted && (
            ...
            {/* Complete shift — only show when a guard is approved and shift isn't already complete */}
            {assignedGuardId && !isCompleted && (
        ...
        <div style={filterGroupStyle}>
            ...
            <div style={filterButtonsStyle}>
                {Object.values(Filter).map(f => (
                    <button key={f} style={selectedFilter === f ? activeFilterButtonStyle : filterButtonStyle} onClick={() => setSelectedFilter(f)}>{f}</button>
                ))}
            </div>
        </div>
...
const gridStyle = { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px', marginBottom: '32px' };
...
const viewDetailsButtonStyle = { backgroundColor: '#274b93', color: 'white', border: 'none', borderRadius: '12px', padding: '12px', fontSize: '14px', fontWeight: '600', cursor: 'pointer', marginTop: '8px' };
...
const feedbackStyle = { marginTop: '8px', marginBottom: '8px', padding: '10px 12px', borderRadius: '10px', backgroundColor: '#f8fafc', color: '#0f172a', border: '1px solid #e2e8f0', fontSize: '13px' };
