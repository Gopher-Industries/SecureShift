...
export const generatePayrollPDF = (
  ...
  if (!records || records.length === 0) {
    throw new Error(
      `No payroll data available to export for ${monthName}.`,
    );
  }
  ...
  doc.text(
    "SecureShift Payroll Summary Report",
    14,
    20,
  );
  ...
  records.forEach((record) => {
    ...
    entries.forEach((entry) => {
      ...
      const shiftTotal = entry.totalAmount || 0;
  ...
  if (tableRows.length === 0) {
    throw new Error(
      `No completed shift entries available to export for ${monthName}.`,
    );
  }
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.text(
      `Page ${i} of ${totalPages} - SecureShift Confidential Record`,
      14,
  ...
  doc.save(
    `SecureShift_Payroll_${monthName}_${fileDate}.pdf`,
  );
};
